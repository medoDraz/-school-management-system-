...
class RolesController extends Controller
{
    ...
    {
        $role = Role::where('accountant',1)->where(function ($q) use ($request) {
            return $q->when($request->search, function ($query) use ($request) {
                return $query->where('name', 'like', '%' . $request->search . '%');
            });
        })->latest()->paginate($request->limit);
        return response()->json(['error' => false, 'message' => __('site.successfully'), 'data' => $role], 200);
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach (explode(',', $request->permissions) as $perm_name){
            if (in_array($perm_name, $perms_name)){
                ...
                $permission = Permission::create(['name' => $perm_name,'accountant'=>1]);
                $role->givePermissionTo($perm_name);
            }
        }
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['error' => true, 'message' => __('site.fill_all_fields'), 'data' => $validator->fails()], 200);
        }
        $perms = Permission::all();
        $perms_name = [];
        foreach ($perms as $p) {
